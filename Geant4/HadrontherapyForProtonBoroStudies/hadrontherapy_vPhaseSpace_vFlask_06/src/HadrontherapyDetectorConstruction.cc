...
void HadrontherapyDetectorConstruction::ConstructPhantom()
{
    ...
    // NEPTUNE: polystyrene flask (CNAO -> dopo il phantom, LNS -> davanti al phantom)
    G4Box* flask = new G4Box("flaskSupport",
                             1*mm/2,
                             phantomSizeY/2,
                             phantomSizeZ/2);
    G4Material* flaskMaterial = G4NistManager::Instance()->FindOrBuildMaterial("G4_POLYSTYRENE", false);
    //G4Material* flaskMaterial = phantomMaterial;
    G4LogicalVolume* flaskLogicalVolume = new G4LogicalVolume(flask,
                                                              flaskMaterial,
                                                              "flaskLog",
                                                              0, 0, 0);
    G4ThreeVector flaskPosition = phantomPosition;
    G4cout << "NEPTUNE::phantomPosition = " <<
        phantomPosition.getX() << ", " <<
            phantomPosition.getY() << ", " <<
                phantomPosition.getZ() << ", " << G4endl;
    flaskPosition.setX(-0.5*mm);
    G4VPhysicalVolume* flaskPhysicalVolume = new G4PVPlacement(0,
                                                               flaskPosition,
                                                               "flaskPhys",
                                                               flaskLogicalVolume,
                                                               motherPhys,
                                                               false,
                                                               0);
    flaskPhysicalVolume -> SetTranslation(flaskPosition);
...
G4bool HadrontherapyDetectorConstruction::SetPhantomMaterial(G4String material)
{
    ...
    // /*
    // Overwrite phantomMaterial (NEPTUNE)
    //////////////////////////////////////
    G4Material* waterMaterial = G4NistManager::Instance()->FindOrBuildMaterial("G4_WATER", false);

    // Boron element with isotope abundance
    G4Isotope* B10 = new G4Isotope("B10",
                                   5, //Z
                                   10, //A
                                   10.*g/mole); //mass of mole
    G4Isotope* B11 = new G4Isotope("B11",
                                   5, //Z
                                   11, //A
                                   11.*g/mole); //mass of mole
    G4Element* myBoron = new G4Element("myBoron",
                                       "B",
                                       2); //n isotopes
    myBoron->AddIsotope(B10,
                        19.9*perCent); //relative abundance
    myBoron->AddIsotope(B11,
                        80.1*perCent); //relative abundance


    // phantom and detector material water + Boron
    phantomMaterial = new G4Material("waterBoron", //name
                                     1*g/cm3, //density
                                     2); // ncomponents
    phantomMaterial->AddMaterial(waterMaterial,
                                 95.0*perCent); //fractionmass
    phantomMaterial->AddElement(myBoron,
                                5.0*perCent); //fractionmass
    detectorMaterial = phantomMaterial;
    G4cout << "NEPTUNE::SetPhantomMaterial phantom and detector Material set" << G4endl;
