...
class RadiobiologyHit : public G4VHit
{
  public:
    RadiobiologyHit();
    RadiobiologyHit(const RadiobiologyHit&);
    virtual ~RadiobiologyHit();

    // Operators
    const RadiobiologyHit& operator=(const RadiobiologyHit&);
    G4int operator==(const RadiobiologyHit&) const;
...
typedef G4THitsCollection<RadiobiologyHit> RadioBioHitsCollection;

// Definition of the Allocator
extern G4ThreadLocal G4Allocator<RadiobiologyHit>* RadioBioHitAllocator;


inline void* RadiobiologyHit::operator new(size_t)
{
  if(!RadioBioHitAllocator)
      RadioBioHitAllocator = new G4Allocator<RadiobiologyHit>;
  return (void *) RadioBioHitAllocator->MallocSingle();
}

inline void RadiobiologyHit::operator delete(void *hit)
{
  RadioBioHitAllocator->FreeSingle((RadiobiologyHit*) hit);
}
